...
from Models.StyleGan2_ada.generator import generator
...
runs_dir = BASE_PATH + 'Training_runs/'

E_ID_LOSS_PATH = BASE_PATH + 'model_ir_se50.pth'
DLIB_WEIGHT_PATH = BASE_PATH + 'mmod_human_face_detector.dat'

TEST_IMAGE_DATA_DIR = BASE_PATH + 'fake/test_image/'
TEST_W_DATA_DIR = BASE_PATH + 'fake/test_w/'

TRAIN_IMAGE_DATA_DIR = BASE_PATH + 'fake/train_image/'
TRAIN_W_DATA_DIR = BASE_PATH + 'fake/train_w/'
...
mlp = LatentMapper()
landmark_encoder = Landmark_Encoder.Encoder_Landmarks(
    MOBILE_FACE_NET_WEIGHTS_PATH)
generator = generator()
...
train_data = Image_W_Dataset(TRAIN_W_DATA_DIR, TRAIN_IMAGE_DATA_DIR)
test_data = Image_W_Dataset(TEST_W_DATA_DIR, TEST_IMAGE_DATA_DIR)


train_loader = DataLoader(
    dataset=train_data, batch_size=config['batchSize'], shuffle=False)
test_loader = DataLoader(
    dataset=test_data, batch_size=config['batchSize'], shuffle=False)

optimizer_D = torch.optim.Adam(discriminator.parameters(
), lr=config['adverserial_D'], betas=(config['beta1'], config['beta2']))
optimizer_adv_M = torch.optim.Adam(mlp.parameters(
), lr=config['adverserial_M'], betas=(config['beta1'], config['beta2']))
...
trainer = Trainer(config, optimizer_D, optimizer_adv_M, optimizer_non_adv_M,
                  discriminator, generator, id_encoder, attr_encoder, landmark_encoder)

run_name = ''.join(choice(ascii_uppercase) for i in range(12))
run = wandb.init(project="ID_disantalgement", reinit=True,
...
with torch.no_grad():
    for idx in range(len(test_ws)):
        ...
        wandb.log({f"Test_ID_Image{idx}": [wandb.Image(
            w_image * 255, caption=f"Test_ID_Image{idx}")]}, step=0)
...
with tqdm(total=config['epochs'] * len(train_loader)) as pbar:
    for epoch in range(config['epochs']):
        ...
        for idx, data in enumerate(train_loader):
            ...
            try:
                with torch.no_grad():
                    id_vec = torch.squeeze(
                        id_encoder.extract_feats((id_images * 2) - 1))
                    real_landmarks, real_landmarks_nojawline = landmark_encoder(
                        attr_images)
            ...
            if use_descrimantive_loss and idx % 2 == 0:
                ...
                wandb.log({'total error': total_error.detach().cpu()},
            ...
            if idx % 1000 == 0 and idx != 0:
                with torch.no_grad():
                    concat_vec = get_concat_vec(
                        test_id_images, test_attr_images, id_encoder, attr_encoder)
                    if config['use_cycle']:
                        concat_vec_cycled = get_concat_vec(
                            test_id_images, test_attr_images_cycled, id_encoder, attr_encoder)

                    with torch.no_grad():
                        for idx in range(len(test_ws)):
                            id_generated_image = get_w_image(
                                mlp(concat_vec)[idx], generator)
                            if config['use_cycle']:
                                cycled_generated_image = get_w_image(
                                    mlp(concat_vec_cycled)[idx], generator)
                            wandb.log(
            ...
            if (idx % 10000) == 0:
                torch.save(
                    mlp, f'{MODELS_DIR}maper_{run_name}_{time.time()}_{int(total_error)}.pt')
                torch.save(
                    attr_encoder, f'{MODELS_DIR}attr_encoder_{run_name}_{time.time()}_{int(total_error)}.pt')
